...
# ╔═╡ feb7f19b-1931-4887-9820-fb7f482ad3db
md"""
### Motivation

#### 0.1 What is climate modeling? 

"""

# ╔═╡ b4aa642d-6968-42a4-8331-6e4e7d706de8
md"""
	
A representation of the exchange of energy between the Earth system and space, and its effects on average surface temperature. (The Climate Laboratory)


"""

# ╔═╡ e6281ecf-d651-4b06-8fea-2c6960777abb
html"""<img src="https://utcdw.physics.utoronto.ca/UTCDW_Guidebook/_images/climate_model_grid.png" height=450>"""

# ╔═╡ 4905f9f8-d2a8-4285-a5b9-baae24f7d8f3
md"""
Source: Earth Magazine and Kotamarthi et al. [2021]
"""

# ╔═╡ 23f662b2-9f5d-4d46-a5ab-e7871cab4ce2
md"""
#### 0.2 What is the difference between climate and weather?

"""


# ╔═╡ 4e9e7bdc-8c43-4c61-9bf7-fa021a398fec
md"""
General definition
...
I like to think of climate being the 'personality' while weather is like 'today's mood.' Climate is much longer time scale than weather.
"""

# ╔═╡ 5e79e7a5-1d18-4c31-bda9-8831ed81001f
begin
	points = 300;
	x = range(0,3,points);
	...
	y_pert2 = y .+ rand(points);
	
	p_1 = plot(x, y_pert1, lw=3, 
		xlabel="Time (years)", ylabel="Surface Temperature", xlabelfontsize=16, ylabelfontsize=16,
...
md"""
#### 0.3 Why care about climate modeling?
...
# ╔═╡ 53e72aa1-c21b-4ae5-983f-34c5311a02fc
md"""
...
# ╔═╡ 47bf7850-d9f5-4866-a604-7ac4f27e74ef
md"""
Data is in the column `"average"`.
"""

# ╔═╡ 3bf6b64c-83ca-47e3-8a93-e22cdd12caba
md"""
Oh no, missing data (-99.99)
"""

# ╔═╡ d60f897a-0d38-456d-a27f-4cce8d7bc66a
validrowsmask = CO2_historical_data_raw[:, "average"] .> 0

# ╔═╡ 2518f5dc-9972-42ba-9f83-0c60433acdb8
CO2_historical_data = CO2_historical_data_raw[validrowsmask,:];

# ╔═╡ 4ab1688f-bf05-4d0c-8c21-a58e02219feb
begin
...
# ╔═╡ 87617bd5-cfa7-42fb-8237-7391364f5d4a
begin
...
#### 2.1 Why is there uncertainty in climate models? 

"""

# ╔═╡ 3c449885-7bb0-4646-903c-997fe61a6397
md"""
Many of us have looked at future climate projections (see figure below) and have thought, "Why is there so much uncertainty in these projections?"
...
md"""
Some coefficients here that have been empirically determined are values $\color{grey} B$, $\color{grey} \text{forcing\_coef}$ and $\color{grey} C$. Let's give those values random numbers within a specified range, and see how our ensemble of 'different models' create a new climate projection.


$\color{grey} B \sim U(0.8,1.8)$
$\color{grey} \text{forcing\_coef} \sim U(4,6)$
$\color{grey} C \sim U(46,56)$


"""

# ╔═╡ 998f79c5-4edd-49de-bcb9-ea78b60e7dd9
# Function to get random variables
function rand_params()
	B_temp = rand(0.8:0.1:1.8)       
	forcing_coef_temp = rand(4:0.1:6) 
	C_temp = rand(46:0.1:56)         
	return B_temp, forcing_coef_temp, C_temp
end

# ╔═╡ 0196cb20-b466-42df-a0d1-b5e5feda1878
begin
	# Let's keep constant for simplicity
	start_temp_temp = 10; # [°C]
	temp₀_temp = 14.0 # preindustrial temperature [°C]
	CO₂_PreIndust_temp = 280.; # preindustrial CO2 concentration [parts per million; ppm];

	# Function accounting for variability in forcing coefficients
	greenhouse_effect_temp(CO₂, forcing_coef_temp) = forcing_coef_temp * log(CO₂/CO₂_PreIndust_temp)
	
	# Function to define the ODE problem
	function define_ODE_problem(B_temp, forcing_coef_temp, C_temp, CO₂)
		return ODEProblem( (temp, p, t)-> (1/C_temp) * ( B_temp*(temp₀_temp-temp)  + greenhouse_effect_temp(CO₂(t), forcing_coef_temp) ) , start_temp_temp,  (0.0, 200) )
	end	

	# Function to define the ODE problem
	function define_ODE_problem_nogreenhouse(B_temp, C_temp)
		return ODEProblem( (temp, p, t)-> (1/C_temp) * ( B_temp*(temp₀_temp-temp) ) , start_temp_temp,  (0.0, 200) )
	end	

	
	# Function 1
	function solve_ensemble(ensemble_size)
	    ensemble_solutions = []
	    for i in 1:ensemble_size
			B_temp, forcing_coef_temp, C_temp = rand_params()
	        problem = define_ODE_problem(B_temp, forcing_coef_temp, C_temp, CO₂)
	        sol = solve(problem)
	...
	# Function 2
	function solve_ensemble_nogreenhouse(ensemble_size)
	    ensemble_solutions = []
	    for i in 1:ensemble_size
			B_temp, ~, C_temp = rand_params()
			# println(B_temp)
	        problem = define_ODE_problem_nogreenhouse(B_temp, C_temp)
	        sol = solve(problem)
	        push!(ensemble_solutions, sol)
	    end
...
md"""
Now, specify how many ensemble members you would like to run.
"""
...
begin
	solutions = solve_ensemble(ens_members);
...
md"""
Another source of uncertainty in the internal climate variability that exists in the chaotic climate system. 
...
begin
	# Function to define the ODE problem
	function define_ODE_problem_with_int_var(B_temp, forcing_coef_temp, C_temp, CO₂)
		return ODEProblem( (temp, p, t)-> (1/C_temp) * ( B_temp*(temp₀_temp-temp)  + greenhouse_effect_temp(CO₂(t), forcing_coef_temp)  + int_var() ) , start_temp_temp,  (0.0, 200) )
	end	

	# Function to solve the ensemble of ODE problems
	function solve_ensemble_with_int_var(ensemble_size)
	    ensemble_solutions = []
	    for i in 1:ensemble_size
			B_temp, forcing_coef_temp, C_temp = rand_params()
	        problem = define_ODE_problem_with_int_var(B_temp, forcing_coef_temp, C_temp, CO₂)
	        sol = solve(problem)
	        push!(ensemble_solutions, sol)
	    end
...
begin
	solutions_with_int_var = solve_ensemble_with_int_var(ens_members_alt);
	
	plot(legend=false, background_color_inside=:black)
	
	for sol in solutions_with_int_var
        plot!(sol.t, sol.u, c=:red)
    end
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash)
	annotate!( 80, temp₀, text("Preindustrial Temperature = $(temp₀)°C",:bottom,color=:white))
...
md"""
*Test different values that control the magnitude of the internal variability. Also, there is no reason that the noise should be additive at the current location. Test at different locations in the equation.*

Between **model uncertainty** and **internal variability**, which is a stronger factor to the overall uncertainty?
...
# ╔═╡ 75e48d52-885b-442d-b285-9fe97e3489bf
md"""
Finally, scenario uncertainty is not uncertainty in the physical laws of the models: it is the uncertainty that results from the social, political and economic factors that are incorporated to make practical estimates on quantities of iterest (QOI), inherently affected by human activity. Think global greenhouse gas emissions. These emissions are directly influenced by human activities. 


"""

# ╔═╡ 923b014e-e156-451b-ab4b-78ea638c14b0
html"""<img src="https://ourworldindata.org/images/published/Greenhouse-gas-emission-scenarios.png" height=450>"""
...
# ╔═╡ 6d9e1941-7b19-453a-aee9-082e155182b7
md"""
Source: Our World in Data
"""

# ╔═╡ d4baac4a-f512-47a5-b835-9a46efe738ad
md"""
There's a whole science behind how socio-political-economic factors affect these projects, which we won't discuss here. 
If you're interested in how these factor into global energy use and climate change, check out this cool website: [En-ROADS](https://en-roads.climateinteractive.org/scenario.html?v=24.9.0)!

We will end by saying scenario uncertainty is also a significant factor to the overall uncertainty we see in climate projections and their applications. 


"""

# ╔═╡ 3de1d205-f753-41f4-a6d7-2fc414eed741


# ╔═╡ 42e0fb37-b214-46f9-9e85-951d1f2001b0
md"""
